...
class NewVerb(AbstractWord):
    ...
    def __eq__(self, other):
        if not isinstance(other, NewVerb):
            return False
    ...
    def capitalize(self):
        ...
        return NewVerb(new_value, self.irregular_past, self.infinitive, self.tags)
    ...
    def bold(self):
        return NewVerb(bold(self.value), self.irregular_past, self.infinitive, self.tags)
    ...
    def past_tense(self):
        ...
        if self.has_tags(WordTag.NEGATIVE):
            return NewVerb("didn't " + self.infinitive, self.irregular_past, self.infinitive, new_tags)

        past_tense_value = self.irregular_past
        if not past_tense_value:
            past_tense_value = add_ed(self.infinitive)
        return NewVerb(past_tense_value, self.irregular_past, self.infinitive, new_tags)
    ...
    def third_person(self):
        ...
        if self.has_tags(WordTag.NEGATIVE):
            return NewVerb("doesn't " + self.infinitive, self.irregular_past, self.infinitive, new_tags)

        with_s = add_s(self.infinitive)
        if with_s == 'haves':
            with_s = 'has'
        return NewVerb(with_s, self.irregular_past, self.infinitive, new_tags)
    ...
    def negative(self):
        ...
        if self.has_tags(WordTag.PAST):
            negative = "didn't "
        return NewVerb(negative + self.infinitive, self.irregular_past, self.infinitive, new_tags)
