...
class TestVerb(unittest.TestCase):
    ...
    def test_verb_repr(self):
        ...
        self.assertEqual(repr(Verb('go', 'went')), "Verb('go', 'went', 'go')")
        self.assertEqual(repr(ThirdPersonVerb('goes', 'went', 'go')), "ThirdPersonVerb('goes', 'went', 'go')")
        self.assertEqual(repr(NegativeVerb("don't go", 'went', 'go')), "NegativeVerb(\"don't go\", 'went', 'go')")
    ...
    def test_to_base_verb(self):
        ...
        third = ThirdPersonVerb('goes', 'went', 'go')
        past = PastVerb('went', 'went', 'go')
        negative = NegativeVerb("don't", 'went', 'go')
        negative_third = NegativeThirdPersonVerb("doesn't go", 'went', 'go')
        negative_past = NegativePastVerb("didn't go", 'went', 'go')
    ...
    def test_Verb_past_tense_irregular(self):
        ...
        self.assertEqual(regular.past_tense(), PastVerb('went', 'went', 'go'))
    ...
    def test_Verb_PastVerb_ThirdPersonVerb_third_person_has(self):
        expected = ThirdPersonVerb('has', 'had', 'have')
        self.assertEqual(expected, Verb('have', 'had').third_person())
        self.assertEqual(expected, ThirdPersonVerb('has', 'had', 'have').third_person())
        self.assertEqual(expected, PastVerb('had', 'had', 'have').third_person())

    def test_NegativeVerb_all_kinds_third_person(self):
        expected = NegativeThirdPersonVerb("doesn't have", 'had', 'have')
        self.assertEqual(expected, NegativeVerb("don't have", 'had', 'have').third_person())
        self.assertEqual(expected, NegativeThirdPersonVerb("doesn't have", 'had', 'have').third_person())
        self.assertEqual(expected, NegativePastVerb("didn't have", 'had', 'have').third_person())

    def test_capitalize(self):
        basic = Verb('go', 'went')
        self.assertEqual(basic.capitalize(), Verb('Go', 'went', 'go'))

        third = ThirdPersonVerb('goes', 'went', 'go')
        self.assertEqual(third.capitalize(), ThirdPersonVerb('Goes', 'went', 'go'))

        past = PastVerb('went', 'went', 'go')
        self.assertEqual(past.capitalize(), PastVerb('Went', 'went', 'go'))

        negative = NegativeVerb("don't go", 'went', 'go')
        self.assertEqual(negative.capitalize(), NegativeVerb("Don't go", 'went', 'go'))

        negative_third = NegativeThirdPersonVerb("doesn't go", 'went', 'go')
        self.assertEqual(negative_third.capitalize(), NegativeThirdPersonVerb("Doesn't go", 'went', 'go'))

        negative_past = NegativePastVerb("didn't go", 'went', 'go')
        self.assertEqual(negative_past.capitalize(), NegativePastVerb("Didn't go", 'went', 'go'))

    def test_repr(self):
        basic = Verb('go', 'went')
        self.assertEqual(repr(basic), "Verb('go', 'went', 'go')")

        third = ThirdPersonVerb('goes', 'went', 'go')
        self.assertEqual(repr(third), "ThirdPersonVerb('goes', 'went', 'go')")

        past = PastVerb('went', 'went', 'go')
        self.assertEqual(repr(past), "PastVerb('went', 'went', 'go')")

        negative = NegativeVerb("don't go", 'went', 'go')
        self.assertEqual(repr(negative), "NegativeVerb(\"don't go\", 'went', 'go')")

        negative_third = NegativeThirdPersonVerb("doesn't go", 'went', 'go')
        self.assertEqual(repr(negative_third), "NegativeThirdPersonVerb(\"doesn't go\", 'went', 'go')")

        negative_past = NegativePastVerb("didn't go", 'went', 'go')
        self.assertEqual(repr(negative_past), "NegativePastVerb(\"didn't go\", 'went', 'go')")
